...
RSpec.describe CommandTower::LoginStrategy::PlainText::Create do
  let(:first_name) { Faker::Name.first_name  }
  let(:last_name) { Faker::Name.last_name  }
  let(:username) { build(:user).username }
  let(:email) { Faker::Internet.email }
  let(:password) do
    min = CommandTower.config.login.plain_text.password_length_min + 1
    max = CommandTower.config.login.plain_text.password_length_max - 1
