...
RSpec.describe CommandTower::UserAttributes::Mutate do
  ...
  describe ".call" do
    ...
    context "with special values" do
      shared_examples "mutate special attribute sharable" do |attribute, success, failed|
      ...
      include_examples "mutate special attribute sharable", :email, Faker::Internet.email, "this is not a valid email"
      include_examples "mutate special attribute sharable", :username, "thisIsAUserName12", "1"
    ...
    context "with no changeable attribute" do
      it "fails with a composition message" do
        expect(call.failure?).to be(true)
        expect(call.msg).to include("Exactly one attribute must be provided for change")
      end
    end
  end
end
