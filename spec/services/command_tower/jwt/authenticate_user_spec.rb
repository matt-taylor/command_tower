...
RSpec.describe CommandTower::Jwt::AuthenticateUser do
  let(:user) { create(:user) }
  let(:token) { CommandTower::Jwt::LoginCreate.(user:).token }
  ...
  describe ".call" do
    ...
    context "with reset" do
      ...
      it "sets expires_at" do
        expect(call.expires_at).to eq(CommandTower.config.jwt.ttl.from_now.to_time.to_s)
      end
