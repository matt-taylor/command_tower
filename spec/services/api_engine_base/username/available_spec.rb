...
RSpec.describe ApiEngineBase::Username::Available do
  describe ".call" do
    subject(:call) { described_class.(username:, force_query:) }

    before do
      ApiEngineBase.config.username.realtime_username_check.local_cache.clear
    end
    ...
    context "when cache is old" do
      before do
        travel = ApiEngineBase.config.username.realtime_username_check.local_cache_ttl.to_i
